...
import { runResearch, type ResearchProgressEvent } from "../researchOrchestrator.js";
...
function mockSdkStepRunner(): SdkResearchStepRunner {
  return async ({ bucket, step, onThreadId }) => {
    onThreadId(`mock-thread-${bucket}`);
    if (step === "partial_evidence" || step === "final_report") {
      return JSON.stringify({
        bucket,
        queries_run: [`mock ${bucket} query`],
        sources_found: [
          {
            bucket,
            title: `Mock ${bucket} source`,
            source_type: bucket,
            url_or_ref: `mock://${bucket}`,
            date_or_version: "mock",
            claim: `Mock evidence for ${bucket}.`,
            confidence: "medium",
            notes: "Generated by the explicit mock worker provider."
          }
        ],
...
export async function processHardflowJob(job: HardflowJob, options: RunJobOnceOptions = {}): Promise<HardflowJob> {
  ...
  if (job.workerProvider !== "codex_sdk" && job.workerProvider !== "mock") {
    return failHardflowJob(job.cwd, job.runId, `worker provider is not implemented for strict research: ${job.workerProvider}`, {
      route: "research",
      researchScope: routeResult.output.researchScope,
      evidenceNeed: routeResult.output.evidenceNeed,
      routerTracePath,
  ...
  const effectiveSdkStepRunner = options.sdkStepRunner ?? (job.workerProvider === "mock" ? mockSdkStepRunner() : undefined);
  const report = await withDaemonWorkerEnv(job.cwd, job.runId, () =>
    runResearch(job.rawUserPrompt, job.cwd, {
      ...
      sdkStepRunner: effectiveSdkStepRunner,
      sdkAvailable: options.sdkAvailable ?? (job.workerProvider === "mock" ? true : undefined),
