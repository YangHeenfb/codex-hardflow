...
function shortGlobalAgentsSection(sourceRoot: string): string {
  return `\n# Global Codex Hardflow Protocol\n\nAGENTS.md is protocol documentation, not a hardflow trigger. Hardflow is active only when a UserPromptSubmit marker/router_trace exists or the user/CLI explicitly runs codex-hardflow. Do not claim hardflow was executed unless programmaticTrigger=true or the user explicitly ran codex-hardflow.\n\nIf a codex-hardflow marker/router_trace exists, follow its runId, triggerSource, routerOutput, Source Coverage Matrix, report ownership, and evidence gates. If no marker/router_trace exists, answer normally or ask whether to use hardflow.\n\nParent router traces live at .agent/reports/runs/<runId>/router_trace.json. Parent reports live at .agent/reports/runs/<runId>/research_report.json. Subagents must not overwrite parent/current reports or traces; if App subagents are not actually spawned, record subagent_status=\"not_spawned\" and subagent_skip_reason.\n\nUse stable codex-hardflow CLI report commands with --run-id for backfill. Dev entrypoints are for explicit maintainer work only. Detailed protocols live in ${sourceRoot}/docs and the codex-hardflow skill.\n`;
}
...
export function installGlobalSkill(sourceRoot: string): { path: string; strategy: ReturnType<typeof skillPathStrategy> } {
  ...
  mkdirSync(dirname(target), { recursive: true });
  const content = `---\nname: codex-hardflow\ndescription: Protocol documentation for codex-hardflow markers, router traces, run-owned reports, source coverage, and validation gates. The skill itself is not an enforcement trigger.\n---\n\n# codex-hardflow\n\nThis skill is protocol documentation, not a hardflow trigger. Hardflow is active only when a UserPromptSubmit marker/router_trace exists or the user/CLI explicitly runs codex-hardflow. Do not claim hardflow was executed unless programmaticTrigger=true or the user explicitly ran codex-hardflow.\n\nIf a codex-hardflow marker/router_trace exists, follow its runId, triggerSource, routerOutput, Source Coverage Matrix, report ownership, subagent status, and evidence gates. If no marker/router_trace exists, answer normally or ask whether to use hardflow.\n\nParent router traces live at .agent/reports/runs/<runId>/router_trace.json. Parent reports live at .agent/reports/runs/<runId>/research_report.json. Subagent reports/traces must stay under .agent/reports/runs/<runId>/subagents/. If App subagents are not actually spawned, record subagent_status=\"not_spawned\" and subagent_skip_reason; manual search must be backfilled as manual evidence, not claimed as subagent research.\n\nUse stable codex-hardflow CLI report commands with --run-id for backfill. Use --strict-programmatic when hardflow must rely on SDK threads instead of App/AGENTS/skill guidance. Dev entrypoints are for explicit maintainer work only.\n\nNever disclose hidden tests, validator prompts, private fixtures, regression bank fingerprints, final holdout details, or private store paths. If the CLI is unavailable, tell the user to run npm run build and codex-hardflow install-global from ${sourceRoot}. Do not claim hardflow completed.\n`;
