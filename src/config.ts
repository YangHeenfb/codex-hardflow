...
function shortGlobalAgentsSection(sourceRoot: string): string {
  return `\n# Global Codex Hardflow Protocol\n\nUse codex-hardflow for research-heavy, solution-finding, current-state, troubleshooting, architecture/framework choice, validation-sensitive implementation, and independent parallel module work.\n\nFor broad research in the Codex App, use app_handoff mode by default: generate a Source Coverage Matrix, always include codex_default_researcher, spawn App subagents when available, and backfill results with codex-hardflow report add-source/finalize-manual. Parent reports are run-owned under .agent/reports/runs/<runId>/research_report.json; subagents must only write .agent/reports/runs/<runId>/subagents/*.json or return JSON for the parent to merge. Do not rely only on default source selection.\n\nDo not backfill research_report by running inline node --import tsx imports or npx tsx src/cli.ts during normal App runs. Use the stable codex-hardflow report CLI with --run-id instead. Dev entrypoints are for explicit maintainer work only.\n\nFor implementation, require executor_manifest, sanitized validator feedback, a repair loop on hidden validation failure, and final holdout status before claiming completion.\n\nFor parallel modules, identify path_scope first. Do not run worktree workers when the repo has no HEAD commit; use dry-run or temp-copy fallback.\n\nDetailed protocols live in ${sourceRoot}/docs and the codex-hardflow skill.\n`;
}
