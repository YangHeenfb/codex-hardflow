...
Use `--runner sdk_threads` or `--execute-sdk-research` only for explicit batch runs where blocking on SDK researcher threads is acceptable. In `strict_programmatic` plus exhaustive mode, default SDK parallelism is `all_required`: `maxConcurrentBuckets` equals the required bucket count unless the user sets a lower value. Current diagnostics support trying all-required parallelism as the default strict exhaustive policy; future telemetry may add adaptive downgrade if stability regresses.
